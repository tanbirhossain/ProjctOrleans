...
namespace Libarius.Network
{
    public class UPnP
    {
        ...
        /// <param name="protocol">Desired protocol.</param>
        /// <param name="description">Description the device should name this new forwarding rule.</param>
        public static void ForwardPort(int externalPort, int localPort, IPAddress localIp, ProtocolType protocol,
            string description)
        {
            if (string.IsNullOrEmpty(_serviceUrl))
                throw new Exception("No UPnP service available or Discover() has not been called");
            var xdoc = SOAPRequest(_serviceUrl,
                ...
                "<NewRemoteHost></NewRemoteHost><NewExternalPort>" + externalPort + "</NewExternalPort><NewProtocol>" +
